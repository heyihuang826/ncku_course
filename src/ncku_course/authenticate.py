...
def verify_session(session, code_ticket, code):
    url = 'https://course-query.acad.ncku.edu.tw/index.php?c=portal&m=robot'
    ...
    headers = {
        ...
        #  'Cookie': f'PHPSESSID={session_cookies["PHPSESSID"]};cos_lang=cht'
         }
...
def authenticate(try_limit : int = 10):
    ...
    for _ in range(try_limit):
        res = session.get(f'https://course-query.acad.ncku.edu.tw/index.php?c=portal&m=robot&{int(time.time())}')
        content = res.content
